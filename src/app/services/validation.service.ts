...
export class ValidationService {
  ...
  static SSN(control: FormControl, field) {
    if (control.value) {
      var regExRules: [
        '^([0-7]\\d{2}|8[0-7]\\d|88[0-9]|89[0-9])([ \\-]?)(\\d{2})\\2(\\d{4})$',
        '^([X]{5})(\\d{4})$'
      ];
      var invalidSSNs = [
        '111111111', '222222222', '444444444', '555555555', '777777777', '888888888', '999999999', '123456789',
        '078051120', '219099999', '987654320', '987654321', '987654322', '987654323', '987654324', '987654325',
      ...
      if (control.value.length == 11) {
        ...
        if (_.indexOf(invalidAreaNumbers, areaNum) !== -1) { valid = false; }
        if (_.indexOf(invalidGroupNumbers, groupNum) !== -1) { valid = false; }
        if (_.indexOf(invalidSerialNumbers, serialNum) !== -1) { valid = false; }
        if (_.indexOf(invalidSSNs, value) !== -1) { valid = false; }
        if (areaNum.substring(0, 1) === '9' || (areaNum.substring(3, 2) === '0' && groupNum.substring(0, 1) === '0')) { valid = false; }
        var re = /^([0-7]\d{2}|8[0-7]\d|88[0-9]|89[0-9])([ \-]?)(\d{2})\2(\d{4})$/;
        var re2 = /^([X]{5})(\d{4})$/;
        if (!re.test(value) && !re2.test(value)) {
          if (_.indexOf(testAreaNumbers, areaNum) === -1) { valid = false; }
        }
        return valid;
      }
    }
  ...
  static phone(control) {
    if (control.value) {
      ...
      var invalidNPA = ['000'];
      var invalidN = ['0', '1'];
      var value = control.value.replace(/-/g, '').replace(/\(/g, '').replace(/ /g, '').replace(/\)/g, '');
      if (10 === value.length) {
        var NPA = value.substring(0, 3);
        var N = value.substring(3, 4);
        var subscriberNum = value.substring(4, 10);
        if (_.indexOf(invalidNPA, NPA) !== -1) { return false; }
        if (_.indexOf(invalidN, N) !== -1) { return false; }
        if (_.indexOf(invalidPhoneNumbers, value) !== -1) { return false; }
      } else {
        return false;
      }
    }
  ...
  static compareFields(control, field) {
    if (control.value && field.formControl) {
      var inValid = false, model = this.orgModel;
